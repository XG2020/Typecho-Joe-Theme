<ul class="links">
    ...
    <?php if ($this->options->JFriends) : ?>
        ...
        if (strpos($this->options->JFriends, '||') !== false) {
            ...
            for ($i = 0; $i < $long; $i++) {
                $list = $list . '<li><div class="item ' . $color[rand(0, 10)] . '" style="background-image:url(' . explode("||", $string_arr[$i])[4] . ');"><div class="title"><a target="_blank" href="' . explode("||", $string_arr[$i])[1] . '">' . explode("||", $string_arr[$i])[0] . '</a></div><div class="content"><div class="desc">' . explode("||", $string_arr[$i])[3] . '</div><img src="' . explode("||", $string_arr[$i])[2] . '" /></div></div></li>';
            }
            echo $list;
        }
        ?>
